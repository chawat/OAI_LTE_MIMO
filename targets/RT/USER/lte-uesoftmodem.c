...
uint8_t nb_antenna_tx = 2;
uint8_t nb_antenna_rx = 2;
...
void set_default_frame_parms(LTE_DL_FRAME_PARMS *frame_parms[MAX_NUM_CCs]) {
  ...
  for (CC_id=0; CC_id<MAX_NUM_CCs; CC_id++) {
    ...
    frame_parms[CC_id]->nb_antenna_ports_eNB  = 2;
...
#  define PACKAGE_VERSION "UNKNOWN-EXPERIMENTAL"
  ...
  for (CC_id=0; CC_id<MAX_NUM_CCs; CC_id++) {
    ...
    frame_parms[CC_id]->nb_antenna_ports_eNB = 2; //initial value overwritten by initial sync later
  }
